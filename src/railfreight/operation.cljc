(ns railfreight.operation
  "OperationActor -- one freight-rail coordination operation = one
  supervised actor run, expressed as a langgraph-clj StateGraph. The
  advisor (RailFreight-LLM) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Rail Freight Governor
  ...
  decision to a human operator (rail operations coordinator). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:schedule-service-operation`/`:flag-track-safety-
  concern`/`:coordinate-maintenance` ALWAYS reach this node when the
  governor is clean -- see `railfreight.phase`."
...
(defn build
  ...
  `railfreight.store/Store`).
  opts:
    :advisor      -- a `railfreight.railfreightllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (railfreightllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; RailFreight-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (railfreightllm/-advise advisor store request)]
            {:proposal p :audit [(railfreightllm/trace request p)]})))

      ;; Rail Freight Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
